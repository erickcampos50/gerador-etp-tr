...
- [ ] Confirm `Gerar HTML imprimivel` remains available while required fields or choices are unresolved, with pending items still signaled in validation and preview.
...
python3 -m py_compile tools/extract_html_model.py tools/validate_model.py
python3 tools/extract_docx_model.py "Documentos modelo/modelo-de-termo-de-referencia-servicos-e-obras-lei-no-14-133-dez-25.docx" --sample 3
python3 tools/extract_docx_model.py "Documentos modelo/modelo-de-termo-de-referencia-servicos-e-obras-lei-no-14-133-dez-25.docx" --model-js /tmp/tr-model-data.js
